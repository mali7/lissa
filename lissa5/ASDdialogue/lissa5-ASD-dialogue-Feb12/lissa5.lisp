...
; clauses" or intepretations based on the user input, which should reveal
...
;      *output-semantics*
;      *output-gist-clauses*
;      *lissa-X-schema* (top-level schema) & possibly many subschemas
...
; the address should change to 
; "c:\ihetpub\wwwnoot\rocspeakrafayet\input.lisp" in Rafayet's version
  ...
  (prog (; INPUT L NAME 
   ...
   ;NAME (SETQ INPUT (READ-WORDS))
	...
	(setq *count* 0)            ; number of outputs so far	
        ...
        ; "structify.lisp".)
        (initialize-plan '*dialog-plan* '*lissa-asd-schema* nil); args = nil
...
;                ~% is (~a ~a ...)~%" {sub}plan-name (car (cddr1 rest))
...
;                            (get plan-name 'schema-name)); DEBUGGING
  ...
  (format t "~%~A~%" (get plan-name 'rest-of-plan))
  (if plan-name
      (return-from initialize-plan plan-name))
 ...
 (let* ({sub}plan-name (rest (get plan-name 'rest-of-plan)) wff)
       ...
       (cond ; First we try to match '(me ...)' (Lissa) actions, typically
           ...
           ((eq (car wff) 'me) ; Lissa action
            (progn (print "~% Before ~%") (implement-next-lissa-action {sub}plan-name) (print "~% After ~%"))) ; also increments
...
; pending action.
;
; * Note - The logic for finding the *deepest* subplan isn't apparent
; from scanning this code. Though I'm probably missing something
; elsewhere. (Unless plan-name can refer to something other than the
; main Lissa dialogue plan.) -PM
;
...
;                 plan-name (car rest) (second rest)); DEBUGGING
       ...
       (cond ((null subplan-name); next action is top-level; it may be 
             ...
             ((null (get subplan-name 'rest-of-plan)); unexpected: If
                                 ...
                                 ; * Note - I've found that this is
                                 ; actually happening. I've updated
                                 ; the code below to account for that
                                 ; circumstance. Worse: when the
                                 ; interpretation arrived at this block,
                                 ; it entered an infinite loop. -PM
              (progn
                  (format t "~%**'find-curr-{sub}plan' applied to ~a ~
                      ~%   arrived at a completed subplan ~a" plan-name
                      subplan-name)
                  (setf (get plan-name 'rest-of-plan) ; Feels like a
                                                      ; hack, but works
                      (cddr (get plan-name 'rest-of-plan)))
                  plan-name))
            ...
            ;(format t "~% **** car expr is ~a **** ~%" (car expr))
             ...
             ((setq bindings
              ...
              (when (null new-subplan-name)
                    ...
                    (return-from implement-next-lissa-action nil))
              (format t "~%~A~%" (get new-subplan-name 'next-action))
...
;                        ~% = ~a " lissa-clauses); DEBUGGING
	       ...
	       ; Make sure that any final puctuation, such as ?, ., or !,
...
; required.
;
; ** ^ On that: first of all, I don't see how it would be meaningful
; or helpful to assign semantics to a subplan name. Second, any user
; response to a reactive comment from Lissa should be handled by other
; (appropriately context-sensitive) mechanisms, and wouldn't be dependent
; on the semantics assigned to the plan itself, unless there's a larger
; point I'm missing. -PM
;
	...
	;(format t "~% ***1*** Tagwords = ~% ~a " tagged-clause); DEBUGGING			              
       ; (format t "~% =====2==== Pattern/output to be matched in rule ~a = ~
        ...
        (cond ((null directive); look depth-first for more specific match
              ...
              ((member directive '(:out :subtrees :gist))
               ...
               (setf (get rule-node 'time-last-used) *count*)
               (return result))
              ; We can't use the instance function on schema objects,
              ; so here's a shortcut that might work -PM
              ; Though I realize that the instance function, or some
              ; version of it, will probably be needed to process the
              ; :schemas directive
              ((member directive '(:schema :schemas :schema+args))
               (setf (get rule-node 'time-last-used) *count*)
               (return (list directive pattern)))
...
;
 ...
 (dolist (word wordlist) 
         (format t "~a " word)
         ; the address should change to 
         ; "c:\ihetpub\wwwnoot\rocspeakrafayet\output.txt" in Rafayet's version
         (with-open-file (outfile "./output.txt" :direction :output :if-exists
                                       :append :if-does-not-exist :create) 
...
;(setq *s* (open "/u/srazavi/LISSA/MePhilipVer/test.txt"))
;(setq *s* (open "/u/srazavi/LISSA/MePhilipVer/test.txt" :direction 
;                    :input :if-exists :new-version :if-does-not-exist nil))
(defun hear-words () 
;````````````````````
      ...
      (progn
        ...
        (if *next-input*
          (progn
            ...
            ;(setq *s* (open "/u/srazavi/LISSA/MePhilipVer/test.lisp" 
            ;                 :direction :output :if-exists :overwrite))
            (with-open-file (outfile "./input.lisp" :direction :output 
                  :if-exists :supersede :if-does-not-exist :create))))))
          ;(write-string "" *s*)
          ;(close *s*)
          ;(delete-file "/u/srazavi/LISSA/MePhilipVer/test.txt")
          ;))
