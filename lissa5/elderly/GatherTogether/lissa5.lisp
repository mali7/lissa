	      ...
	      (setq expr (second (car bindings))); e.g., '(I am a ...)
            ...
            ;(format t "~% **** car expr is ~a **** ~%" (car expr))
             ...
             ((setq bindings
              ...
              ;(format t "~%*** Step1 ~%" )
			  (format t "~% bindings is ~a ~%" bindings)
              (format t "~% wff is ~a ~%" wff)
...
;
 (let (user-gist-words choice tagged-words wff subplan-name 
      ...
      (cond ((null (cdr user-gist-clauses)); single clause?
             ...
             ;(format t "~% (single clause) tagwords are ~a ~% " tagged-words)
             (setq choice (choose-result-for tagged-words 
                                           '*reaction-to-input*))
			 ;(format t "~% (single clause) choice are ~a ~% " choice)  
			 )
...
;
 (let (wordstring)
	  ...
	  ;(with-open-file (lockfile "./lock" :if-does-not-exist :create)
 	  (setf *default-pathname-defaults* *root-dir*)
	  (with-open-file (outfile "./output.txt" :direction :output :if-exists
