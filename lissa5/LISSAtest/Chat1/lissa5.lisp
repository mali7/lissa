...
(with-open-file (outfile "./kb.txt" :direction :output :if-exists 
                               :supersede :if-does-not-exist :create))
)
)
;(format t "~%hi1~%")
...
;                        ends in a LISSA action name" wff1); DEBUGGING
             ...
             ; (format t "~% MMMMMMMMlissa clauses = ~a" (car (last lissa-clauses)))
	       ...
	       ; than "'", "_", or "-"]
		       ;; AAAAA HEREEEE masalan if input = 123 then 
               (when (null input)
                     (delete-current-action {sub}plan-name)
                     (return-from observe-next-user-action nil))
...
;
 (let (user-gist-words choice tagged-words wff subplan-name 
      ...
      (cond ((null (cdr user-gist-clauses)); single clause?
             ...
             ;(format t "~% BUUUUUUUUUUtagwords are ~a ~% " tagged-words)
             (setq choice (choose-result-for tagged-words 
                                           '*reaction-to-input*))
			 ;(format t "~% BUUUUUUUUUUUchoice are ~a ~% " choice)  
			 )
            ...
            (T ; there are multiple gist clauses;
			   ...
			   ;(format t "~% NOTEEEEEEEEEEEEEEE user-gist-words are ~a ~% " user-gist-clauses)
               (setq user-gist-clauses_p (purify_func user-gist-clauses))
			   ; here:  if there exist multiple gist clasuse, remove those with "nothing found"
			   ; if you want to add gist clauses to the global gist-kb you could do it here
			   ;(format t "~% user-gist-words are ~a ~% " user-gist-clauses_p)
...
(defun purify_func1 (user-gist-clauses)
(if (null user-gist-clauses) 
    nil 
    (if (and (eq (caar user-gist-clauses) 'Nothing) (eq (cadar user-gist-clauses) 'found)) 
        (purify_func1 (cdr user-gist-clauses))
	    (cons (car user-gist-clauses) (purify_func (cdr user-gist-clauses)))))
)

(defun purify_func (x)
(if (and (eq (length x) 1) (eq (caar x) 'Nothing) (eq (cadar x) 'found))
   (return-from purify_func x)
   (purify_func1 x))
)
    ...
    ;(format t tagged-clause)	
        ...
        (cond ((null directive); look depth-first for more specific match
            ...
            ;;                      rule-node pattern tagged-clause parts))
 			;(if (null newparts)
			;(format t "~% next rule = ~a ~%" (get rule-node 'next)))
     ...
     ; (format t "~% tagged prior gist clause = ~a" tagged-prior-gist-clause)
      ...
      (dolist (chunk chunks)
         ...
         (when clause ; this is of form (wordlist keys)
            ...
            (store-fact specific-answers keys *gist-kb*))
        ...
        (while clause
            (progn
			(setq clause1 (list (first clause) (second clause)))
            (setq keys (second clause1))
            (push (pop clause1) unbidden-answers)
            (store-fact unbidden-answers keys *gist-kb*)
      ...
      (when thematic-answer
         ...
         (store-fact (list thematic-answer) keys *gist-kb*))
...
(defun print-hash-entry-into-file (key value)
   (setq value-list (car value))
   (setq value-list (remove-duplicates value-list :test #'equal))
   (while value-list 
        (progn
		    ;(format t "~%value-list = ~a ~%" value-list)
	        (if (not (and (eq (caar value-list) 'NOTHING) (eq (cadar value-list) 'FOUND)))
                (with-open-file (outfile "./kb.txt" :direction :output :if-exists
                                 :append :if-does-not-exist :create)
                                (format outfile "~a~%" (car value-list)))
			)
...
(defun store-facts-into-file (kb)
(maphash #'print-hash-entry-into-file kb)   
) ; end of store-facts-into-file
...
(defun say-words (wordlist); Apr 12/16; NB: there's a PRINT-WORDS fcn too
;```````````````````````````
...
;
 (let (wordstring)
      ...
      (with-open-file (outfile "./output.txt" :direction :output :if-exists
                                :append :if-does-not-exist :create)
                      (format outfile "~%#~D: ~a" *lissa-count* wordstring))
