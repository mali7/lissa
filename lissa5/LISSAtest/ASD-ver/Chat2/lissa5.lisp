...
;
 (let ((n (length words)) tagged-prior-gist-clause relevant-trees 
      ...
      ;(format t "~% prior-gist-clause = ~a" prior-gist-clause)
...
;
 (let (wordstring)
	  ...
	  (with-open-file (lockfile "./lock" :direction :io :if-exists :error  :if-does-not-exist :create)
 	   (with-open-file (outfile "./output.txt" :direction :output :if-exists
...
;; IF YOU WANT TO TRY THIS SEPARATELY, YOU ALSO HAVE TO DEFINE
;; *lissa-count* AND THE 'EXPLODE' FUNCTION
  ...
  (let (words feats feat)
       ...
       (loop (setq feat (pop feats)); current word/feature
             ...
             (setq feats (union (get feat 'feats) feats)); add feats of
                                                         ; current word
            ; Debugging for the potential feat loop
			; (format t "~%feat is = ~a ~%" feat)
			; (format t "~%feats are = ~a ~%" feats)
