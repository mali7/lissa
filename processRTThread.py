...
if __name__ == '__main__':
	...
	newest = ""
	old_merge_webm = ""
	bin64dir = "C:/Program Files/Visage Technologies/visageSDK/bin64/"
	while(1):
		newest = max(glob.iglob('C:/inetpub/wwwroot/RocSpeakRafayet/uploads/*.wav'), key=os.path.getctime)
		newest_merge_webm = max(glob.iglob('C:/inetpub/wwwroot/RocSpeakRafayet/uploads/*merge.webm'), key=os.path.getctime)
		#print newest[:-4]
		...
		if newest!=old :
			for i in xrange(8):
				...
				if newest[:-4] == newest_webm[i][:-5]:
					...
					print "not_found " 
		if newest_merge_webm!= old_merge_webm:
			old_merge_webm = newest_merge_webm
			command = "cd "+bin64dir+" & AnalyzeVideo2.exe "+newest_merge_webm+ " 2>&1"
			try:
			   thread.start_new_thread( process, (command, ) )
			except:
			   print "Error: unable to start thread"
