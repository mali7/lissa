  ...
  if (!('webkitSpeechRecognition' in window)) {
      ...
      recognition.onend = function() {
		...
		if (final_transcript.length > 1){
			...
			xhr.open('GET', "response.php?action=speechsavelisp&value="+final_transcript+"&keyNumber="+keyNumber+"&dataKey="+dataKey, true);
			xhr.send();
		}
		///////////////////////////////////////////////////////////
        if (speechRecognitionIsOn){
...
function agingRecordingStart(){
	...
	setInterval(function(){
		if (recording==1) agingRecordingEnd();
	},30000);
}
...
function keydown(event){
	...
	if(event.which == 75 || event.which == 33 || event.which == 34 || event.which == 32){
		if(isdown == 0){
			isdown = 1;
			if(speechRunnign == 0){
				speechRunnign = 1;
...
function keyup(event){
	...
	if(event.which == 67 || event.which == 66){
		//speechNum-=1;
		stopSpeech();
		// loadScene(07);
		// setGaze(180,1000,15);
...
function displayDataRealTime(displayValue)
{
	...
	dataReadyFileNames[dataReadyFileNamesCount] = key;
	dataReadyFileNamesCount++;
	// var xhr = new XMLHttpRequest();
...
{
	checkGetDataRealTimer=setInterval(function(){
		console.log("in.....................checkgetdatarealtime checkGetDataCount= "+checkGetDataCount+" CheckArrayCount = "+CheckArrayCount+" processDoneArrayCount = "+processDoneArrayCount );
		...
		xhr.onreadystatechange = function() {
			if (this.readyState == 4) {
				if (this.status == 200) {
					...
					console.log("Failed while sending request to check data.");
					if(CheckArrayCount>processDoneArrayCount){
						CheckArrayCount=processDoneArrayCount;
					}
					if((processRecordingCount-checkGetDataCount>=2)||(CheckArrayCount<processDoneArrayCount-1)){
			...
			if (this.readyState == 4) {
				if (this.status == 200) {
					...
					else if(this.responseText==responseNotToProceed)
					{
						...
						console.log("process recording 2");
					}
				} else {
				//console.log("Failed while sending request to process data.");
				//insertLightboxMessage('Please continue to wait as we analyze your data.');
...
//while(1)
	console.log("split video record starting");
     splitTimer=setInterval(function(){
            ...
            if(recording==0 && splitRecordControl%3 == 0){
                    //start 
...
*/
  request.onreadystatechange = function () {
    if (this.readyState == 4) {
      if (this.status == 200) {
        splitRecorder.uploadFileToServerCallback(this.responseText);
      } else {
        console.log("Error message: "+this.responseText);
        splitRecorder.uploadFileToServerErrorCallback();
      }
    } else {
