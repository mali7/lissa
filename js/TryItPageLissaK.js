...
var keyNumber;
var mineyeResponse;
var minsmileResponse;
var minbodyResponse;
var minvolumeResponse;
...
function initialize() {
  ...
  if (!('webkitSpeechRecognition' in window)) {
      ...
      recognition.onend = function() {
        console.log("ended recognition I am here 1");
		var x1 = final_transcript;
		console.log("X1 = "+x1);
		...
		showspeech.style.fontSize = "large";
		showspeech.textContent = "Your speech: "+final_transcript;
		showspeech.textContent = "";
		showspeech.style.color = "#c78737";
		if (final_transcript.length > 1){
			//showspeech.textContent = final_transcript;
			xhr.open('GET', "response.php?action=speechsavelisp&value="+final_transcript+"&keyNumber="+keyNumber, true);
			xhr.send();
		}
		///////////////////////////////////////////////////////////
        if (speechRecognitionIsOn){
      ...
      recognition.onresult = function(event) {
		...
		// if(interim_transcript.length == 0 && final_transcript.length >1){
			// console.log("Here we should save="+final_transcript);
			// var xhr = new XMLHttpRequest();
			// xhr.open('GET', "response.php?action=speechsavelisp&value="+final_transcript+"&keyNumber="+keyNumber, true);
			// //showspeech.textContent = "Participant: "+final_transcript;
			// final_transcript = '';
			// xhr.send();
			
		// }
        console.log("From Onresult final_transcript",final_transcript,"interim_transcript",interim_transcript);
    };
  }

 doGetUserMedia();
}
function start_speech_recognizer(){
	console.log("Now recognize");
	//showspeech.textContent = "Lissa is listening";
	keydown(75);
	keyup(75);
}
function findkey(){
...
function startRecordingAfterActive() {
	...
	document.getElementById("postfeedback").disabled = false;
    //recognition.start();
}

function keydown(event){
	console.log("key value"+event.which);
	if(event.which == 75 || event.which == 33 || event.which == 32){
		if(isdown == 0){
			isdown = 1;
			if(speechRunnign == 0){
				speechRunnign = 1;
				...
				recognition.start();
				showspeech.style.fontSize = "x-large";
				showspeech.textContent = "I am listening";
				showspeech.style.color = "#00CCCC";
				//showspeech.style.fontSize = "large";
			}
			else{
				speechRunnign = 0;
...
function keyup(event){
	...
	if(event.which == 67 || event.which == 66|| event.which == 34){
		//speechNum-=1;
		stopSpeech();
		// loadScene(07);
		// setGaze(180,1000,15);
		stopRecordingOnHangup();
		window.location="lissak.php?keyNumber="+keyNumber+"&speechNum="+speechNum;
	}
}
...
function submitInstruction(){
	...
	var xhr = new XMLHttpRequest();
			xhr.onreadystatechange = function() {
				if (this.readyState == 4) {
					if (this.status == 200) {
						console.log("Lisp Result "+this.responseText);
						instruction.value = this.responseText;
						
					}
				}	
			}
	xhr.open('GET', "response.php?action=submitInstruction&value="+inst+"", true);
	xhr.send();
}
...
function displayDataRealTime(displayValue)
{
	...
	var Key=dataKey.toString()+"split"+displayValue+"";
	var xhr = new XMLHttpRequest();
	xhr.open('GET', "response.php?action=savefilenameLissa&dataKey="+Key+"&private="+isPrivateModeOn, true);
	xhr.send();
				...
				if (this.status == 200) {
					...
					if(this.responseText==responseToProceedcheck){
						...
						if (checkGetDataCount<=processDoneArray[processDoneArrayCount-1]){
							...
							xhr.open('GET', "response.php?action=checkformatteddataLissa&dataKey="+Key+"&private="+isPrivateModeOn, true);
							xhr.send();
						}
					}
					else if(this.responseText==responseNotToProceedcheck)
	...
	processRecordingRealTimer=setInterval(function(){
		...
		xhr.onreadystatechange = function() {
			if (this.readyState == 4) {
				if (this.status == 200) {
					...
					//	console.log("Just checking processRecordingCount="+processRecordingCount+" splitCount="+splitCount);
						if (processRecordingCount<splitCount){
							...
							xhr.open('GET', "response.php?action=process2lissa&dataKey1="+Key1+"&dataKey2="+Key2+"&dataKey3="+Key3+"&dataKey4="+Key4, true);
							xhr.send();
						}
					}
					else if(this.responseText==responseNotToProceed)
					{
						if(splitCount-processRecordingCount > 3) processRecordingCount++;
						var Key1=dataKey.toString()+"split"+processRecordingCount+"";
						var Key2=dataKey.toString()+"split"+(processRecordingCount+1)+"";
						var Key3=dataKey.toString()+"split"+(processRecordingCount+2)+"";
						var Key4=dataKey.toString()+"split"+(processRecordingCount+3)+"";
				...
				var Key4=dataKey.toString()+"split"+(processRecordingCount+3)+"";
				setTimeout(function(){
					console.log("Again here................................processeing file "+Key1);
					xhr.open('GET', "response.php?action=process2lissa&dataKey1="+Key1+"&dataKey2="+Key2+"&dataKey3="+Key3+"&dataKey4="+Key4, true);
					xhr.send();
				},3000);
			}
...
//while(1)
     ...
     splitTimer=setInterval(function(){
		// doGetUserMedia();
		console.log("splitCount....."+splitCount+" recording="+recording+" splitRecordControl = "+splitRecordControl+" dataKey = "+dataKey);
		splitRecordControl++;
		if(recording==1){
				//stop 
				//recording=0;
		...
		if(recording==0 ){
				...
				splitRecordControl = 0 ;
				splitRecorder=new MRecordRTC();
				// console.log("3.here recording="+recording);
				splitRecorder.addStream(localStream);
				splitRecorder.startRecording();
				// console.log("4.here recording="+recording);
				recording=1;
				// console.log("5.here recording="+recording);
				splitCount++;
		}
    }, 3000);
...
function stopRecordingOnHangup() {
	...
	startButton.className = "btn btn-primary btn-lg";
	//mergeVideos();
	if(maxbodystreak<bodystreak)maxbodystreak=bodystreak;
	if(maxsmilestreak<smilestreak)maxsmilestreak=smilestreak;
	if(maxvolumestreak<volumestreak)maxvolumestreak=volumestreak;
	if(maxeyestreak<eyestreak)maxeyestreak=eyestreak;
	if(mineyeResponse==10000)mineyeResponse=0;
	if(minvolumeResponse==10000)minvolumeResponse=0;
	if(minsmileResponse==10000)minsmileResponse=0;
	if(minbodyResponse==10000)minbodyResponse=0;
	var streaks=maxeyestreak+" "+maxvolumestreak+" "+maxsmilestreak+" "+maxbodystreak+"\n";
	var popcounts=eyePopCount+" "+volumePopCount+" "+smilePopCount+" "+bodyPopCount+"\n";
	var responseCount= mineyeResponse+" "+minvolumeResponse+" "+minsmileResponse+" "+minbodyResponse+"\n";
	console.log("streaks = "+maxeyestreak+" "+maxvolumestreak+" "+maxsmilestreak+" "+maxbodystreak);
	console.log("minResponse = "+ mineyeResponse+" "+minvolumeResponse+" "+minsmileResponse+" "+minbodyResponse);
	var xhr1 = new XMLHttpRequest();
	console.log("keylabel = "+keyNumber);
	xhr1.open('GET', "response.php?action=streakandCount&keyNumber="+keyNumber+"&streaks="+streaks+"&popcounts="+popcounts+"&responseCount="+responseCount, true);
	xhr1.send();
	clearInterval(wizozTimer);
	speechRecognitionIsOn = false;
	stopped=1;
