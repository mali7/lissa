...
var myWhammyStartTime, myWhammyEndTime;
var myOwnWhammyStartTime, myOwnWhammyEndTime;
var myAudioVideoTimeDifference;
...
function MediaStreamRecorder(mediaStream) {
    console.log("used MediaStreamRecorder");
...
function StereoRecorder(mediaStream) {
    console.log("used StereoRecorder");
    this.record = function() {
...
function StereoAudioRecorder(mediaStream, root) {
    ...
    this.stop = function() {
        ...
        // flat the left channel down
        var interleaved = mergeBuffers(leftchannel, recordingLength);
        
        console.log("audio length",interleaved.length);
        myAudioLength = interleaved.length;
    ...
    if (sampleRate >= 44100) sampleRateDivisor = 4;
    else sampleRateDivisor = 2;
    sampleRate = sampleRate/sampleRateDivisor;
    
    myAudioSampleRate = sampleRate;
    ...
    console.log("numberOfInputs",__stereoAudioRecorderJavacriptNode.numberOfInputs);
    console.log("numberOfOutputs",__stereoAudioRecorderJavacriptNode.numberOfOutputs);
    bufferSize/=sampleRateDivisor;
    
    console.log('new sample-rate', sampleRate);
    console.log('new buffer-size', bufferSize);
    ...
    __stereoAudioRecorderJavacriptNode.onaudioprocess = function(e) {
        ...
        downsample = new Float32Array(left.length/sampleRateDivisor);
        for (var i=0; i<downsample.length;i++) {
            downsample[i] = 0;
            for (var j=0;j<sampleRateDivisor;j++) {
                downsample[i] += left[i*sampleRateDivisor+j];
            }
            downsample[i] /= sampleRateDivisor;
        }

        // we clone the samples
        //leftchannel.push(new Float32Array(left));
        leftchannel.push(downsample);
...
function WhammyRecorder(mediaStream) {
    ...
    this.record = function() {
        ...
        (function drawVideoFrame(time) {
            ...
            if (!myWhammyStartTime) {
                myWhammyStartTime = time;
                myOwnWhammyStartTime = Date.now();
                console.log("myWhammyStartTime",myWhammyStartTime);
            }
            ...
            if (lastFrameDuration == 0) {
                lastFrameDuration = time - lastFrameTime;
                //console.log("skip frame");
            } else {
                whammy.add(canvas, time - lastFrameTime + lastFrameDuration);
                lastFrameDuration = 0;
                //console.log("keep frame");
            }
            // whammy.add(canvas);

            lastFrameTime = time;
            myWhammyEndTime = time;
            myOwnWhammyEndTime = Date.now();
        })();
    };
    ...
    this.record = function() {
        ...
        function drawVideoFrame(time) {
            ...
            if (time - lastFrameTime < 90) {
                //console.log("time - lastFrameTime < 90",time - lastFrameTime);
                return;
            }
...
var Whammy = (function() {
    ...
    function toWebM(frames) {
        ...
        console.log("toWebM(frames)",frames.length);//mfung
        myVideoFrameCount = frames.length;
        var info = checkFrames(frames);
        //console.log("info1",info);
        /*
        var myCorrectedInfo = myFrameRateCorrection(frames);
        info = myCorrectedInfo["info"];
        frames = myCorrectedInfo["frames"];
        //console.log("info2",info);
        //console.log("frames2",frames);
*/
        console.log("frame count",frames.length,"duration",info.duration);

        var myCorrectedInfo = myFrameRateCorrection2(frames);
        frames = frames.slice(myCorrectedInfo.numFramesToDrop);
        info.duration = myCorrectedInfo.duration;
        console.log("frame count",frames.length,"duration",info.duration);
    ...
    function checkFrames(frames) {
        ...
        for (var i = 1; i < frames.length; i++) {
            ...
            //console.log("duration",duration,"timespan",frames[i].duration);
        }
        
        
        //mfung start
        //myVideoFrameRateMultiplier = (1000.0*myAudioLength/myAudioSampleRate)/duration;
        console.log("audio duration",1000.0*myAudioLength/myAudioSampleRate,"video duration",duration,"whammy duration",myWhammyEndTime - myWhammyStartTime,"my own whammy duration",myOwnWhammyEndTime - myOwnWhammyStartTime);
        console.log("target multiplier",(1000.0*myAudioLength/myAudioSampleRate)/duration);
        myAudioVideoTimeDifference = duration - (1000.0*myAudioLength/myAudioSampleRate);
        //mfung emd
    ...
    /*
    function myFrameRateCorrection(frames) {
        var width = frames[0].width,
            height = frames[0].height,
            duration = myVideoFrameRateMultiplier*frames[0].duration;
        frames[0].duration = myVideoFrameRateMultiplier*frames[0].duration;
        console.log("duration",duration,"timespan",frames[0].duration);
        console.log("myVideoFrameRateMultiplier",myVideoFrameRateMultiplier,frames[1].duration,myVideoFrameRateMultiplier*frames[1].duration);
        for (var i = 1; i < frames.length; i++) {
            frames[i].duration = myVideoFrameRateMultiplier*frames[i].duration;
            duration += frames[i].duration;
            //console.log("duration",duration,"timespan",frames[i].duration);
        }
        //console.log("myFrameRateCorrection",frames);
        console.log("audio duration",1000.0*myAudioLength/myAudioSampleRate,"new video duration",duration);
        return {
        "frames":frames,
        "info":{
            duration: duration,
...
*/
    ...
    function myFrameRateCorrection2(frames) {
        var width = frames[0].width,
            height = frames[0].height;
        console.log("myAudioVideoTimeDifference",myAudioVideoTimeDifference);
        var duration, skippedDuration = 0.0,numFramesToDrop = 0;
        if (myAudioVideoTimeDifference>0) {
            duration = 0.0;
            skippedDuration = frames[0].duration;
            numFramesToDrop += 1;
        } else {
            duration = frames[0].duration;
        }
		
		//videoTimesMillisecs.push(duration);
        for (var i = 1; i < frames.length; i++) {
            if (myAudioVideoTimeDifference > skippedDuration && myAudioVideoTimeDifference > 0) {
                numFramesToDrop += 1;
                skippedDuration += frames[i].duration;
                console.log("skipped frame");
            } else {
                duration += frames[i].duration;
            }
            console.log("i",i,"duration",duration,"frames[i].duration",frames[i].duration);
			//videoTimesMillisecs.push(duration);
        }
        return {
            numFramesToDrop:numFramesToDrop,
            duration:duration
        };
    }
