...
<script src="js/TryItPageLissaK.js"></script>
...
<div id= "textdiv" style="position:fixed ; width:15%;height:30%; bottom: 40%; right:0%"> </div>
...
function wizoz(){
	var messageGot=0;
	var backColor=0;
	var talking = 0; //this needs to be removed.
		...
		xhr.onreadystatechange = function() {
			if (this.readyState == 4) {
				if (this.status == 200) {
					...
					if(this.responseText[0]=='1' && talking==0){
					...
					if(this.responseText[2]=='1' && talking==0){
					...
					if(this.responseText[3]=='1' && talking==0){
		...
		xhr1.onreadystatechange = function() {
			if (this.readyState == 4) {
				if (this.status == 200) {
					...
					console.log("inside speechNum = "+speechNum);
					var lines = this.responseText.split("\n");
					//console.log("hello======"+lines[0]);
					for (var i = speechNum; i<lines.length;i++){
